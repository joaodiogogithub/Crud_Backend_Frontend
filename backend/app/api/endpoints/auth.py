...
@router.post(
    ...
    description="(OAuth2 compatible token / endpoint), get an access token for future requests using username and password",
)
async def login_access_token_oauth2(
...
@router.post(
    ...
    description="Get an access token for future requests using refresh token",
)
async def access_token_jwt(
    ...
    if user is None:
        # this is naive method to not return early
        verify_password(data.password, DUMMY_PASSWORD)
